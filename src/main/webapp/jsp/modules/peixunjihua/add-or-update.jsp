            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑培训计划</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="zhigongId" name="zhigongId" type="hidden">
                                    <div class="form-group col-md-6 peixunjihuaUuidNumberDiv">
                                        <label>培训计划编号</label>
                                        <input style="width: 450px" id="peixunjihuaUuidNumber" name="peixunjihuaUuidNumber" class="form-control"
                                               placeholder="培训计划编号">
                                    </div>
                                    <div class="form-group col-md-6 peixunjihuaNameDiv">
                                        <label>培训计划标题</label>
                                        <input style="width: 450px" id="peixunjihuaName" name="peixunjihuaName" class="form-control"
                                               placeholder="培训计划标题">
                                    </div>
                                    <div class="form-group col-md-6 peixunjihuaTypesDiv">
                                        <label>培训计划类型</label>
                                        <select style="width: 450px" id="peixunjihuaTypesSelect" name="peixunjihuaTypes" class="form-control">
                                        </select>
                                    </div>
                                    ...
                                    <div class="form-group  col-md-6 peixunjihuaContentDiv">
                                        <label>培训内容</label>
                                        <input id="peixunjihuaContentupload" name="file" type="file">
...
<script>
    ...
    function upload() {
        ...
        $('#peixunjihuaContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('peixunjihuaContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("peixunjihua/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationPeixunjihuatypesSelect(){
            var peixunjihuaTypesSelect = document.getElementById('peixunjihuaTypesSelect');
            if(peixunjihuaTypesSelect != null && peixunjihuaTypesOptions != null  && peixunjihuaTypesOptions.length > 0 ){
                for (var i = 0; i < peixunjihuaTypesOptions.length; i++) {
                    peixunjihuaTypesSelect.add(new Option(peixunjihuaTypesOptions[i].indexName,peixunjihuaTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(peixunjihuaTypesSelect != null && peixunjihuaTypesOptions != null  && peixunjihuaTypesOptions.length > 0 ) {
            for (var i = 0; i < peixunjihuaTypesOptions.length; i++) {
                if (peixunjihuaTypesOptions[i].codeIndex == ruleForm.peixunjihuaTypes) {//下拉框value对比,如果一致就赋值汉字
                        peixunjihuaTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.peixunjihuaContent != null && ruleForm.peixunjihuaContent != 'null' && ruleForm.peixunjihuaContent != '' && $("#peixunjihuaContentupload").length>0) {
    ...
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#peixunjihuaContentupload").length>0) {
            var peixunjihuaContentEditor = UE.getEditor('peixunjihuaContentEditor');
            if (peixunjihuaContentEditor.hasContents()) {
                $('#peixunjihuaContent-input').attr('value', peixunjihuaContentEditor.getContent());
            }
        }
    }
    //数字检查
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                zhigongId: "required",
                peixunjihuaUuidNumber: "required",
                peixunjihuaName: "required",
                peixunjihuaTypes: "required",
                kaishiTime: "required",
                jieshuTime: "required",
                peixunjihuaContent: "required",
            },
            messages: {
                zhigongId: "职工不能为空",
                peixunjihuaUuidNumber: "培训计划编号不能为空",
                peixunjihuaName: "培训计划标题不能为空",
                peixunjihuaTypes: "培训计划类型不能为空",
                kaishiTime: "培训开始时间不能为空",
                jieshuTime: "培训结束时间不能为空",
                peixunjihuaContent: "培训内容不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addpeixunjihua = window.sessionStorage.getItem("addpeixunjihua");
        if (addpeixunjihua != null && addpeixunjihua != "" && addpeixunjihua != "null") {
            //注册表单验证
            $(validform());
            $("#peixunjihuaUuidNumber").val(new Date().getTime()+Math.ceil(Math.random()*10));//设置唯一号
            ...
            window.sessionStorage.removeItem('updateId');
            http("peixunjihua/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#peixunjihuaUuidNumber").val(ruleForm.peixunjihuaUuidNumber);
        $("#peixunjihuaName").val(ruleForm.peixunjihuaName);
        $("#kaishiTime-input").val(ruleForm.kaishiTime);
        $("#jieshuTime-input").val(ruleForm.jieshuTime);
        $("#peixunjihuaContent").val(ruleForm.peixunjihuaContent);
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationPeixunjihuatypesSelect();
            <!--  初始化级联表的下拉框  -->
